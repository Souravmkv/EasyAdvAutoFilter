...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
                        ...
                        await client.send_cached_media(
                            ...
                            reply_markup=InlineKeyboardMarkup(
                                ...
                                [
                                    ...
                                    InlineKeyboardButton("🌿 Mᴏᴠɪᴇ Sᴇᴀʀᴄʜ Gʀᴏᴜᴘ 🥀", url="t.me/saurav7even")
                                ]
                                ]
                            )
                        )
        ...
        buttons = [[
                    ...
                    InlineKeyboardButton('😎 ᴅᴇᴠᴇʟᴏᴘᴇʀ', url="t.me/saurav7even"),
        ...
        buttons = [[
            InlineKeyboardButton('✉️ ᴀɴʏ ǫᴜᴇʀʏ ✉️', url='https://t.me/sauravjoshi2007')
        ], [
        ...
        await query.message.edit_text(
            ...
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
                InputMediaPhoto("https://graph.org/file/bd2980632a829d4f21564.jpg")
            )
            reply_markup = InlineKeyboardMarkup(btn)
            await query.message.edit_text(
                text="⋘ Pʟᴇᴀsᴇ Wᴀɪᴛ ⋙"
...
async def auto_filter(client, msg, spoll=False):
    ...
    if settings["button"]:
        btn = [
            [
                InlineKeyboardButton(
                    text=f"[{get_size(file.file_size)}] {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.') and not x.startswith('tg') and not x.startswith('telegram'), file.file_name.split()))}", callback_data=f'{pre}#{file.file_name}'
                ),
            ]
            for file in files
        ]
